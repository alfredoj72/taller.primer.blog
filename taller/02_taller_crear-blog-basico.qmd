...
Vamos a  crear el blog con R y RStudio; concretamente trabajaremos utilizando un `Rproject`, así que el primer paso para crear nuestro blog es **crear un Rproject** donde se alojaran los ficheros que necesitemos para nuestro blog.

::: {.callout-important icon="false"}
#### Tarea 1: Creación del Rproject que generará el blog

-   En RStudio sigue la siguiente ruta de menús:   

    `File > New Project > New Directory > Quarto Blog`

- Al hacerlo se creará una carpeta, un Rproject, con todo lo necesario para crear un blog.

-   El nombre que le he puesto a la carpeta/Rproject es `blog_prueba_00`
:::

De momento aún no tenemos el blog, solo hemos creado un Rproject pero, ya tenemos, dentro de la carpeta del Rproject, la estructura de carpetas y archivos necesarios para crearlo.

Cuando creemos el blog, será un blog sencillito, vamos con lo mínimo, pero un blog. 

Antes de crear el blog hagamos un pequeño repaso al contenido del Rproject, veamos su estructura de carpetas y archivos.

<br>

## Contenido del Rproject

El Rproject que acabamos de crear y que permitirá crear el blog tiene esta estructura de archivos y carpetas:


::: {#fig-01 layout-ncol=1}
![](./imagenes/tt_01/img_01.png){fig-align="center"}

Estructura del Rproject `blog_prueba_00`
:::



Como ves en la figura anterior, el Rproject contiene **6 archivos y una subcarpeta**. Voy a dividirlos en dos categorías:   
...
No es un archivo importante para el blog. Este archivo simplemente permite que RStudio reconozca la carpeta `blog_prueba_00` como un Rproject.
:::
...
Nos quedan 3 archivos que será en los que nos concentremos ahora mismo, bueno, enseguida.
...
Un archivo `.qmd` que generará el archivo `about.html` y que será una de las páginas de nuestro blog; precisamente en esta página se vera nuestra foto.
:::
...
El Rproject que hemos creado tiene todo lo necesario para crear un blog, pero aún no lo hemos creado. Para crearlo solo tienes que: 
...
- De esta forma, se procesarán los archivos del Rproject y se creará el blog.
...
- En ambos casos hay que estar en el Rproject "blog_prueba_00"; es decir, "blog_prueba_00" es el working directory
...
Tras haber creado el blog, la estructura de carpetas de nuestro Rproject ha cambiado, concretamente se han generado 2 subcarpetas. Veámoslo:
...
- Como crear un Rproject con la estructura de carpetas y archivos necesaria para generar un blog básico

- Hemos visto (de forma rápida) el contenido, los archivos, que hay en el Rproject. 

- El Rproject contiene 3 archivos que serán los importantes, los que trabajaremos en el próximo tutorial, concretamente:  `_quarto_yml`, `index.qmd` y `about.qmd`
