...
#- https://quarto.org/docs/websites/website-blog.html#themes
    ...
    toc-depth: 3
    toc-location: left
    toc-title: Estructura del tutorial nº 3
...
-   Como ves, `index.qmd` es un fichero (nuestra "Home Page") sin contenido aparente: aparte del `yaml`no hay nada más. Sin embargo, justamente en el `yaml` especifica, con la opción `listing:` (linea 3), que la página va a ser un listado; concrétamente un listado del contenido que haya en la carpeta `posts`; o sea, un listado con los posts que haya en la carpeta `posts`.
...
- Simplemente he cambiado la linea 1. Aunque casi queda mejor, para mi gusto, comentar (con un #) para que no haya ningún título, casi queda mejor sin título.
...
- Puedes cambiar la apariencia estética del blog y dejarlo a tu gusto usando los `themes` prefedinidos en Quarto o puedes usar el archivo `styles.css`^[Veremos en el tutorial nº 8 que también podremos usar archivos.scss]

- De momento, el archivo `styles.css` está vacio^[En realidad no está vacio, pero solo tiene un comentario.]


- El tuneado del blog con .css lo veremos con algo más de detalle en el tutorial nº 8, ahora mismo solo haremos unas modificaciones mínimas: voy a justificar el texto^[Esta es una modificación que tiene muchas criticas, pero, que se le va a hacer, a mi me gusta el texto jsutificado.].
...
- Finalmente hemos modificado (minímamente) el archivo `styles.css`.
